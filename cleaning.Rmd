...
clean_poly_everything <- inner_join(polygons, survey_clean, by = "resp_id") #success! this is in class sf
...
#for some reason when you join things to spatial dataframes it loses its sf classification, need to re-add

clean_poly_everything <- st_as_sf(x = clean_poly_everything, sf_column_name = "geometry") 

write_csv(clean_poly_everything,"H:/punjab_project/punjab_project/clean_poly_survey.csv", col_names = TRUE )
...
Since all of these polygons say "no burn", just see if any of them come as burned using the first raster, char threshold = 0.9
