...
for(i in seq_along(unique(df_short$unique_id))){
  ...
  poly <- st_sf(data.frame(unique_id = id, st_sfc(st_polygon(list(as.matrix(df_short))),crs = 4326)))
...
#change column name of poly_sf from id to unique id so they match
...
#if this doesn't work, than just do a table join in arcmap
