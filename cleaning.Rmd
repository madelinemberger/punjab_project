...
#use `anti_join` from the tidverse to take out the 200 sample, just need to convert sf objects to dataframes 

clean_done_df <- as.data.frame(clean_done)

clean_sample_df <- as.data.frame(clean_sample)

all_minus_sample <- anti_join(clean_done_df, clean_sample_df, by = "unique_id")

#convert back to spatial object
all_minus_sample_sf <- st_as_sf(x = all_minus_sample, sf_column_name = "geometry")
